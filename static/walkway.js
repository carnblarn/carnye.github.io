...
(function (root, factory) {
    ...
    document.addEventListener(
        ...
        function () {
            ...
            for (
                var i = 0, instancesLen = _instances.length;
                i < instancesLen;
                i++
            ) {
                _instances[i].cancel();
            ...
            for (
                var j = 0, elementsLen = _elements.length;
                j < elementsLen;
                j++
            ) {
    ...
    Walkway.prototype = {
        ...
        draw: function (callback) {
            ...
            while (counter--) {
                ...
                if (Date.now() - this.begin < element.startTime) {
                    continue;
                }
                done = element.update();
    ...
    function Path(path, duration, easing) {
        ...
        this.startTime = ((0.5 * parseFloat(startX)) / 3000) * this.duration;
    }
    ...
    function Polyline(polyline, duration, easing) {
        ...
        var startX = polyline.points.getItem(0).x;
        this.startTime = ((0.5 * parseFloat(startX)) / 3000) * this.duration;
    }

    Path.prototype = Line.prototype = Polyline.prototype = Object.create(
        WalkwayElement.prototype
    );
