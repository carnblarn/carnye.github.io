...
;(function(root, factory) {
  ...
  function Walkway(opts) {
    ...
    for(var i = 0; i < this.paths.length; i ++ ){
      ...
      if(this.paths[i].el.tagName == "line") x = this.paths[i].el.getAttribute('x1');
      else if(this.paths[i].el.tagName == "polyline") x = this.paths[i].el.points[0].x;
      else if(this.paths[i].el.tagName == "path") x = this.paths[i].el.pathSegList ? this.paths[i].el.pathSegList.getItem(0).x;
      this.paths[i].animationStartTime = timer +  x* 5;
    }
  ...
  function generalUpdate(element) {
    if(!element.animationStarted && Date.now() < element.animationStartTime){
      return false;
    }
